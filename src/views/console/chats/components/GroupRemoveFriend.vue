      ...
      <div style="display: flex">
        <div style="width: 300px;">
          ...
          <div style="max-height: 420px; overflow: hidden">
            <el-checkbox-group v-for="item in list" :key="item.username" v-model="selected">
        ...
        <div style="flex: 1;">
          <div style="height: 30px">
            <div style="font-size: 14px;float: left">删除成员</div>
...
const list = ref([])
onMounted(() => {
  Object.keys(store.groupMember[store.operateUsername]).forEach(username => {
    if(username !== store.userInfo.username) {
      list.value.push(store.groupMember[store.operateUsername][username])
    }
  })
})
